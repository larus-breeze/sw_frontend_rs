use super::{DrawColored, Rotate, SCALE_MARKER_PCOORDS};
...
impl DrawColored for ScaleMarker {
            ...
            D: DrawTarget<Color = Colors, Error = CoreError>
    {
        ...
        let px = [
            SCALE_MARKER_PCOORDS[0].to_xy(scale, rotation) + center,
            SCALE_MARKER_PCOORDS[1].to_xy(scale, rotation) + center,
            SCALE_MARKER_PCOORDS[2].to_xy(scale, rotation) + center,
        ];
