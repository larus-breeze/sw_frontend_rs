...
impl CoreModel {
    ...
    pub fn nmea_plarv(&mut self) -> &str {
        ...
        let _ = uwrite!(
            ...
            self.sensor.gps_altitude.to_m(),    // Fixme: insert pressure altitude when available
    ...
    /*pub fn nmea_plars(&mut self, id: SysConfigId) -> &str {
        let (short, val) = match id {
            SysConfigId::MacCready => ("MC", self.config.mc_cready.to_m_s()),
            SysConfigId::WaterBallast => ("BAL", self.glider_data.ballast_ratio()),
            SysConfigId::Bugs => ("BUGS", (self.glider_data.bugs - 1.0) * 100 ),
            SysConfigId::Qnh => ("QNH", )
                let ballast = 
            }
        }
...
mod tests {
    use crate::{
        basic_config::MAX_TX_FRAMES, model::GpsState, AirSpeed, Coord, CoreModel, FloatToDensity, FloatToLength, FloatToSpeed, HwVersion, Latitude, Longitude, QIdleEvents, QTxFrames, SwVersion, WindVector
    };
    ...
    fn plarv() {
        ...
        cm.sensor.gps_altitude = 1234.0.m();
        cm.sensor.airspeed = AirSpeed::from_tas_at_nn(111.1.km_h());
        let s = cm.nmea_plarv();
        assert_eq!(s, "$PLARV,2.50,1.25,1234,111*6D\r\n");
    }
    ...
    fn gprmc() {
        ...
        let s = cm.nmea_gprmc();
        assert_eq!(s, "$GPRMC,120520.00,A,4941.39652,N,835.06958,E,123.4,321.4,230623,,,A*53\r\n");
    }
