...
use crate::{flight_physics::Polar, model::EditMode, utils::KeyEvent, CoreModel, POLARS};
...
impl CoreController {
    ...
    pub fn time_action(&mut self, core_model: &mut CoreModel) {
        ...
        let climb_rate = core_model.measured.climb_rate;
        let mc_cready = core_model.calculated.mc_cready;
        let sink_rate = self.polar.sink_rate(core_model.measured.airspeed);
        ...
        core_model.calculated.speed_to_fly_dif =
            core_model.calculated.speed_to_fly.ias() - core_model.measured.airspeed.ias();
        ...
        match self.tick {
            1 => self
                .polar
                .recalc(&core_model.glider_data, core_model.measured.density),
            _ => (),
        }
    }
