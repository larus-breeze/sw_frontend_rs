...
use crate::{
    ...
    model::{DataSource, DisplayActive, EditMode, OverlayActive, VarioModeControl},
...
impl CoreController {
    ...
    fn tick_100ms(&mut self, core_model: &mut CoreModel) {
        ...
        if core_model.config.display_active == DisplayActive::DeviceInfo
            && core_model.config.overlay_active == OverlayActive::None
        {
            match core_model.control.alive_ticks % 30 {
                0 => persist::send_can_config_frame(
                    core_model,
                    self,
                    CanConfigId::AntBaselen,
                    RemoteConfig::Get,
                ),
                10 => persist::send_can_config_frame(
                    core_model,
                    self,
                    CanConfigId::AntSlaveDown,
                    RemoteConfig::Get,
                ),
                20 => persist::send_can_config_frame(
                    core_model,
                    self,
                    CanConfigId::AntSlaveRight,
                    RemoteConfig::Get,
                ),
                _ => (),
            }
