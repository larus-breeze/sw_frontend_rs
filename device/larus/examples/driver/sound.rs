...
#[allow(unused)]
const TRIANGULAR_WAVE: [u16; SAMPLES_CNT] = [2251, 2456, 2661, 2865, 3070, 2865, 2661, 2456, 2251, 2046, 1842, 1637, 1432, 1228, 1023, 1228, 1432, 1637, 1842, 2046];
#[allow(unused)]
const SAWTOOTH_WAVE: [u16; SAMPLES_CNT] = [2149, 2251, 2354, 2456, 2558, 2661, 2763, 2865, 2968, 3070, 1126, 1228, 1331, 1433, 1535, 1638, 1740, 1842, 1945, 2047];
#[allow(unused)]
const RECTANGULAR_WAVE: [u16; SAMPLES_CNT] = [3070, 3070, 3070, 3070, 3070, 3070, 3070, 3070, 3070, 3070, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024, 1024];
#[allow(unused)]
const SINE_WAVE: [u16; SAMPLES_CNT] = [2363, 2648, 2875, 3020, 3070, 3020, 2875, 2648, 2363, 2047, 1730, 1445, 1218, 1073, 1023, 1073, 1218, 1445, 1730, 2046];
...
impl Sound {
    ...
    fn set_wave(&mut self, sound_on: bool) {
        ...
        let dma1 = unsafe { &(*pac::DMA1::ptr()) };
        dma1.st[0].cr.modify(|_, w| w.en().clear_bit()); // start dma transfer
        dma1.st[0].m0ar.write(|w| unsafe { w.bits(wave_ptr as u32) }); // src
        dma1.st[0].cr.modify(|_, w| w.en().set_bit()); // start dma transfer
    }
}
