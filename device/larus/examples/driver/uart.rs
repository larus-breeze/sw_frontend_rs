...
impl NmeaTxRx {
    ...
    pub fn new(
        ...
        let _serial = usart1
            .serial((tx, rx), 38_400.bps(), prec, clocks)
            .unwrap();
        ...
        unsafe {
            ...
            dma1.st[2].ndtr.write(|w| w.ndt().bits(NMEA_BUF_SIZE as u16)); // buf len
    ...
    pub fn read(&mut self) -> Option<&[u8]> {
        if self.head == self.tail {
            ...
            if self.head > self.tail {
                self.tail = self.head;
                unsafe {
                    Some(&RX_BUFFER[start..self.head])
                }
            } else { // head < tail
                self.tail = 0;
                unsafe {
                    Some(&RX_BUFFER[start..NMEA_BUF_SIZE])
                }
            }   
        }
    }
