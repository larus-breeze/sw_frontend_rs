...
impl LcdInterface {
    ...
    pub fn new<'a>(
        ...
        config_bcr(bcr);
        let bcr = unsafe {core::mem::transmute::<&Bcr3, &Bcr1>(&fsmc.bcr3)};
        config_bcr(bcr);
        let bcr = unsafe {core::mem::transmute::<&Bcr4, &Bcr1>(&fsmc.bcr4)};
        config_bcr(bcr);

        config_btr(&fsmc.btr1, read_timing);
        let btr = unsafe {core::mem::transmute::<&Btr2, &Btr1>(&fsmc.btr2)};
        config_btr(btr, read_timing);
        let btr = unsafe {core::mem::transmute::<&Btr3, &Btr1>(&fsmc.btr3)};
        config_btr(btr, read_timing);
        let btr = unsafe {core::mem::transmute::<&Btr4, &Btr1>(&fsmc.btr4)};
        config_btr(btr, read_timing);

        config_bwtr(&fsmc.bwtr1, write_timing);
        let bwtr = unsafe {core::mem::transmute::<&Bwtr2, &Bwtr1>(&fsmc.bwtr2)};
        config_bwtr(bwtr, write_timing);
        let bwtr = unsafe {core::mem::transmute::<&Bwtr3, &Bwtr1>(&fsmc.bwtr3)};
        config_bwtr(bwtr, write_timing);
