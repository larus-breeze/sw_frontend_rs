...
impl Persistence {
    ...
    pub fn new(i2c: I2c<I2C1>) -> Result<Self, Error> {
        ...
        eeprom
            .read_data(EEPROM_IDENTIFICATION_BLOCK, &mut magic)
            .map_err(|_| Error::EepromOrI2c1)?;
        delay_ms(10);
        if magic != EEPROM_MAGIC {
            // Write magic number
            eeprom
                .write_page(EEPROM_IDENTIFICATION_BLOCK, &EEPROM_MAGIC)
                .map_err(|_| Error::EepromOrI2c1)?;
            delay_ms(10);
...
fn main() -> ! {
    ...
    persist
        .write_item(PersistentItem::from_f32(PersId::Ballast, 1.1))
        .unwrap();
    persist
        .write_item(PersistentItem::from_f32(PersId::Bugs, 1.0))
        .unwrap();
    persist
        .write_item(PersistentItem::from_f32(PersId::MacCready, 1.3))
        .unwrap();
    persist
        .write_item(PersistentItem::from_f32(PersId::Qnh, 1013.2))
        .unwrap();
