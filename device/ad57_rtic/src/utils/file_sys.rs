...
impl FileSys {
    /// Create FileSys
    pub fn new(dp_sdio: SDIO, clocks: &Clocks, pins: SdioPins) -> Self {
        let mut fs = None;
        let sdio: Sdio<SdCard> = Sdio::new(dp_sdio, pins, clocks);
        if let Ok(fileio) = FileIo::new(sdio) {
            if let Ok(fs_) = FileSystem::new(fileio, FsOptions::new()) {
                fs = Some(fs_);
            }
        }
        ...
        if file_sys.find_image() {
            match file_sys.copy_image() {
                Ok(()) => {
                    file_sys.update_available =
                        FirmwarUpadate::Available(file_sys.meta_data.sw_version)
                }
    ...
    pub fn copy_image(&mut self) -> Result<(), DevError> {
        if let Some(fs) = &self.fs {
            ...
            NorFlash::erase(
                &mut unlocked_flash,
                flash_offset as u32,
                (flash_offset + self.image_size) as u32,
            )
            .unwrap();

            loop {
                let b_read = image_file.read(&mut buf)?;
                NorFlash::write(&mut unlocked_flash, flash_offset as u32 + bytes_read, &buf)
                    .unwrap();
            ...
            let upper_flash_u32 =
                unsafe { core::mem::transmute::<usize, &[u32; 0x2_0000]>(BEGIN_UPPER_FLASH) };
            let new_app_end_idx =
                self.meta_data.new_app - BEGIN_UPPER_FLASH + self.meta_data.new_app_len;
    ...
    fn find_image(&mut self) -> bool {
        if let Some(fs) = &self.fs {
            ...
            let meta_data_as_u8arr = unsafe {
                core::mem::transmute::<&mut MetaData, &mut [u8; META_DATA_SIZE]>(
                    &mut self.meta_data,
                )
            };
            ...
            if self.meta_data.magic == 0x1c80_73ab_2085_3579
                && self.meta_data.sw_version != SW_VERSION
            {
